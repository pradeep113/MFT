...
if st.session_state["logged_in"]:
    ...
    if workflow_options == "Create Webuser":
        st.write(f"You selected: {workflow_options}")
    ...
    # Sidebar widgets
    Health_stats = st.sidebar.selectbox("Health Stats:", ["Services and Connections", "Recent Executions", "Expirations", "Obsolete"])
    # Main content
    if Health_stats == "Services and Connections":
        st.write(f"You selected: {Health_stats}")
    if Health_stats == "Recent Executions":
        st.write(f"You selected: {Health_stats}")
    if Health_stats == "Expirations":
        st.write(f"You selected: {Health_stats}")
    if Health_stats == "Obsolete":
        st.write(f"You selected: {Health_stats}")
